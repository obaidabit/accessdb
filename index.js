...
function connect(){
    ...
    client.on('error', function(err) {
        log('error.log',__filename.slice(__dirname.length + 1)+" at "+new Error(err).stack);
        ...
        switch (msg.path) {
            ...
            case "/balance":
                stm = `SELECT valueN,(select sum([القيمة]) from [دفعات] ${
                    msg.date ? "where [التاريخ] <= #" + msg.date + "#" : ""
